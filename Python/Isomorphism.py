...
def set_difference_isomorphs(graph1, graph2):
    result = []
...
def compare_rules_in_paired_datasets(dataset_big, dataset_small, path_to_rules, path_to_datasets):
    ...
    for model in models:
        ...
        graph_difference = set_difference_isomorphs(graph1=graph_by_head_big, graph2=graph_by_head_small)
        ...
        print(
            f"Model:{model}; Number of rules in {dataset_big}: {len(graphs_big)}; Number of rules in {dataset_small}: {len(graphs_small)}; Number of rules in the difference: {len(graph_difference)}; Number of intersections:{len(graph_intersection)}; Number of unions:{len(graph_union)}; Ratio:{len(graph_intersection) * 1.0 / len(graph_union)}")
